...
fn report_convergence(book: &Book, dict: &Dictionary, signal: &Signal) {
    ...
    let used: Vec<String> = hist
        ...
        .map(|(i, &c)| match dict.blocks[i].env.params {
            rmp_core::Shape::Fof(p) => format!("a{:.0}/b{:.1}:{c}", p.alpha, p.beta * 1000.0),
            rmp_core::Shape::Gaussian(g) => format!("s{:.1}:{c}", g.sigma * 1000.0),
        })
        .collect();
...
fn report_roundtrip(book: &Book, signal: &Signal) {
    let resynth = rmp_synthesis::atoms::render_atoms(book, signal.len()).unwrap();
    let err: f64 = signal
        .samples
        .iter()
        .zip(&resynth.samples)
    ...
    println!(
        ...
        rmp_core::signal::snr_db(signal.energy(), err)
    );
}
