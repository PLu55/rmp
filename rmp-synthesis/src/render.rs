...
use rmp_core::residual::book::{ResidualBook, RESIDUAL_BOOK_VERSION};
use rmp_core::signal::Signal;
use crate::atoms;
use crate::bank::{BankCalibration, SynthesisBand, SynthesisBank};
use crate::config::{ClippingPolicy, OutputEncoding, RenderConfig};
use crate::error::RenderError;
use crate::gain::BandGainState;
use crate::rng::{NoiseSource, Xoshiro256pp};
...
/// [`rmp_core::book::read_doc`], which owns the extension rules.
pub fn load_book(path: &Path) -> Result<BookInput, RenderError> {
    match rmp_core::book::read_doc::<Book>(path) {
        Ok(b) => {
        ...
        Err(full_err) => match rmp_core::book::read_doc::<ResidualBook>(path) {
            Ok(r) => {
                r.validate()
                    .map_err(|e| RenderError::Book(format!("{}: {e}", path.display())))?;
...
mod tests {
    use super::*;
    use crate::testing::{a_book, atom_book, book_with_power};
    ...
    fn a_single_active_band_is_localised_at_its_centre() {
        use rmp_core::residual::filter::{AnalysisBand, GammatoneBand};
    ...
    fn a_longer_smoothing_time_softens_the_attack() {
        use crate::config::GainSmoothingConfig;
    ...
    mod files {
        ...
        fn fof(t0: i64, f: f32, amp: f32) -> rmp_core::fof::AtomParams {
            rmp_core::fof::AtomParams {
                t0,
                f,
                env: rmp_core::fof::EnvelopeParams::new(800.0, 0.001).into(),
                phi: 0.2,
                amp,
            }
        }

        fn gauss(t0: i64, f: f32, amp: f32) -> rmp_core::fof::AtomParams {
            rmp_core::fof::AtomParams {
                t0,
                f,
                env: rmp_core::gauss::GaussianParams::new(0.002).into(),
                phi: -0.4,
                amp,
            }
        }
        ...
        fn a_book_of_either_kind_is_recognised_from_the_document() {
            ...
            for ext in ["toml", "json", "json.gz"] {
                ...
                rmp_core::book::write_doc(&p, &residual).unwrap();
                ...
                rmp_core::book::write_doc(&p, &full).unwrap();
                match load_book(&p).unwrap() {
